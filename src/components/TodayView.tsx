    ...
    <div className="flex flex-col bg-white dark:bg-gray-900">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-white dark:bg-gray-900 shadow-lg pb-[env(safe-area-inset-bottom)]" style={{ transform: 'translateZ(0)' }}>
        ...
        <div className="max-w-4xl mx-auto px-4 py-2 md:py-3">
          ...
          {/* Action buttons row - Save (prominent) on LEFT, Date and Reset on RIGHT */}
          <div className="flex items-center justify-between gap-3">
            {/* Left: Save button - PRIMARY, PROMINENT */}
            <button
              onClick={handleSave}
              disabled={!hasUnsaved || saveLoading}
              className={`px-4 md:px-6 py-2 md:py-2.5 rounded font-bold whitespace-nowrap transition flex items-center justify-center gap-2 flex-shrink-0 text-sm md:text-base min-h-10 md:min-h-11 ${
                saveError
            ...
            {/* Right: Date and Reset button */}
            <div className="flex items-center gap-2 md:gap-3">
              {/* Date label */}
              <div className="text-xs md:text-sm text-gray-500 dark:text-gray-500 whitespace-nowrap">
                {new Date(date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}
              </div>

              {/* Reset button - SECONDARY, SMALL */}
              <button
                onClick={handleResetToDefault}
                className="px-3 md:px-4 py-1 md:py-1.5 border border-gray-300 dark:border-gray-600 rounded text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 whitespace-nowrap transition flex-shrink-0 text-xs md:text-sm"
              >
                Reset
              </button>
            </div>
